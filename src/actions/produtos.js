...
export const startRemoveProduto = ({ id } = {}) => {
    return (dispatch, getState) => {
        const uid = getState().auth.uid
        return database.ref(`users/${uid}/produtos/${id}`).remove().then(() => {
...
export const startEditProduto = (id, updates) => {
    return (dispatch, getState) => {
        const uid = getState().auth.uid
        return database.ref(`users/${uid}/produtos/${id}`).update(updates).then(() => {
...
export const startSetProdutos = () => {
    return (dispatch, getState) => {
        ...
        return database.ref(`users/${uid}/produtos`).once('value').then((snapshot) => {
            const produtos = []
