...
export const startAddVenda = (vendaData = {}) => {
    return (dispatch, getState) => {
        ...
        const venda = { numero, cliente, produtos, subTotal, total, frete, desconto, observacoes, status, dataVenda }
        database.ref(`users/${uid}/vendas`).push(vendaData).then((ref) => {
            dispatch(addVenda({
                id: ref.key,
                ...venda
            }))
...
export const startRemoveVenda = ({ id } = {}) => {
    return (dispatch, getState) => {
        const uid = getState().auth.uid
        return database.ref(`users/${uid}/vendas/${id}`).remove().then(() => {
...
export const startEditVenda = (id, updates) => {
    return (dispatch, getState) => {
        const uid = getState().auth.uid
        return database.ref(`users/${uid}/vendas/${id}`).update(updates).then(() => {
...
export const startSetVendas = () => {
    return (dispatch, getState) => {
        ...
        return database.ref(`users/${uid}/vendas`).once('value').then((snapshot) => {
            const vendas = []
