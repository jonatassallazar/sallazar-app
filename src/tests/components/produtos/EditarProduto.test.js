...
it('should delete and navigate back to vendas', async () => {
  ...
  render(<EditarProduto match={{ params: { id: '123abc' } }} history={history} />, {
    initialState,
  });
...
describe('testing base functions', () => {
  ...
  it('should find product by id', () => {
    ...
    expect(screen.getByTestId('nome-produto').childNodes[1].childNodes[0]).toHaveValue(
      'Purse'
    );
  });
...
describe('should handle all submit actions', () => {
  ...
  it('should send data to onSubmit on props', () => {
    render(<EditarProduto history={history} match={{ params: { id: '123abc' } }} />, {
      initialState,
    });
